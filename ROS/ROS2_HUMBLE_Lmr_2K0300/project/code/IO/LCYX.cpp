...
bool ServoController::setAngle(int angle) {
    ...
    // }
    if (angle < 0) {
        std::cerr << "警告：角度小于0度，已将其设置为0度。" << std::endl;
        angle = 0; // 将小于0度的角度设置为0度
    } else if (angle > 180) {
        std::cerr << "警告：角度大于180度，已将其设置为180度。" << std::endl;
        angle = 180; // 将大于180度的角度设置为180度
    }
// 如果角度在0到180之间，则什么也不做，继续使用原值
    // 计算新的占空比
