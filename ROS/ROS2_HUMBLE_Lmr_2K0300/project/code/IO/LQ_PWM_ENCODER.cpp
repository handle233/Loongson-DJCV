...
LS_PwmEncoder::LS_PwmEncoder(int channel, int gpio)
{
    this->Direction = gpio;
...
float LS_PwmEncoder::Update(void)
{
    //LCYX
    // 读取全缓冲寄存器的值
    // 由于编码器的脉冲周期是从0开始计数的，所以
    // 如果上次读取的值和这次读取的值一样，说明没有新脉冲，
    // 此时速度应为0
    // 这里使用了一个静态变量 last_val 来保存上次读取的值
    // 这样可以避免每次调用 Update() 时都要读取一次全缓冲
    // 寄存器的值，从而提高效率
    static uint32_t last_val = 0;
    //LCYX
    uint32_t val = LS_readl(this->Full_buffer);
    //LCYX
    // 如果val和上次一样，说明没有新脉冲，速度应为0
    // if (val == last_val) {
    //     return 0.0f;
    // }
    // last_val = val;
    //LCYX
    (val == 0) ? (val = 1) : (val); // 防止除0
    return val;
    // return (float)(100000000.0 / val / NUM_ENCODER_LINE * (this->Direction * 2 - 1));
}
